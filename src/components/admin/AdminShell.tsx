    ...
    <AdminGuard>
      <main className="min-h-screen bg-[#050607] px-4 py-6 pb-32 text-white md:px-8">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[240px_1fr]">
          <aside className="rounded-lg border border-white/10 bg-[#090b0d] p-4 lg:sticky lg:top-6 lg:h-fit">
            <button
              type="button"
              onClick={() => setIsMenuOpen((current) => !current)}
              className="flex w-full items-center gap-3 rounded-lg border border-white/10 bg-white/[0.03] p-3 text-left transition hover:border-cyan-300/40 hover:bg-white/10"
            >
              <LogoMark />
            ...
            <Link
              href={homeItem.href}
              className="mt-3 flex items-center gap-3 rounded-md px-3 py-3 text-sm font-semibold text-zinc-300 transition hover:bg-white/10 hover:text-cyan-200"
            >
